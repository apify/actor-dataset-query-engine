# type: ignore
"""
Example of a JSON analyze query engine using SQLite and defaults prompts
"""
...
class JSONAnalyzeQueryEngineWorkflow(Workflow):
    ...
    async def jsonalyzer(self, ctx: Context, ev: StartEvent) -> JsonAnalyzerEvent:
        ...
        try:
            ...
            msg = 'sqlite-utils is needed to use this Query Engine:\npip install sqlite-utils'
        ...
        return JsonAnalyzerEvent(sql_query=sql_query, table_schema=table_schema, results=results)
...
llm_ = OpenAI(model='gpt-4o-mini')

w = JSONAnalyzeQueryEngineWorkflow()

# Run a query
query_ = 'What is the maximum age among the individuals?'


async def main() -> str | None:
    r = await w.run(query=query_, list_of_dict=json_list, llm=llm_, table_name=DEFAULT_TABLE_NAME)
...
# Draw all
draw_all_possible_flows(JSONAnalyzeQueryEngineWorkflow, filename='flow_all.html')
